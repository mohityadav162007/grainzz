...
export default function EditBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (blog?.featured_image_url) form.append('existing_image', blog.featured_image_url);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-black text-gray-900">Edit Blog Post</h1>
          <p className="text-gray-500 text-sm">Update your blog article content</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input 
                  name="sort_order" 
                  type="number" 
                  defaultValue={blog?.sort_order || 0} 
                  className="admin-input" 
                />
              </div>
            </div>
              ...
              <div className="mt-1 flex items-center gap-4">
                <div className="w-32 h-32 rounded-xl bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden relative group">
                  {imageFile ? (
                    ...
                    <img src={blog.featured_image_url} alt="" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="text-xs text-gray-500">
                  <p className="font-bold text-gray-700 mb-1">Click image to change (optional)</p>
            ...
            <div className="flex items-center gap-3 pt-4">
              <input 
                ...
                type="checkbox" 
                value="true" 
                defaultChecked={blog?.is_active} 
                className="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary" 
              />
              <label className="text-sm font-semibold text-gray-700">Published (Visible on website)</label>
            </div>
          </div>
        </div>
        <div className="admin-card p-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving} className="admin-btn h-11 px-8">
            {saving ? <><Loader2 size={18} className="animate-spin" /> Saving...</> : 'Update Blog Post'}
          </button>
